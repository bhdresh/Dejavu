      ...
      <div class="form-group has-feedback" style="width: 300px">
        ...
        <span class="glyphicon fa fa-laptop form-control-feedback"></span>
         ...
         <!-- /.box -->
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Container Network Subnet</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <form role="form" action="updateSettings.php" method="post">
                <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>"/>
              <?php
	             exec("sudo /usr/bin/sipcalc docker0|grep -i \"Network mask\"|grep -iv \"bits\" |head -1|awk -F \"- \" '{print$2}'|xargs",$outputmask,$result);
       	             exec("sudo /usr/bin/sipcalc docker0|grep -i \"Host address\"| head -1|awk -F \"- \" '{print$2}'",$outputnetworkip,$result);

             	     $dockermask=$outputmask[0];
                     $dockerip=$outputnetworkip[0];

              ?>
      <div class="form-group has-feedback" style="width: 350px">
        <input type="text" name="dockerip" class="form-control" placeholder="Container Network IP (Default: 172.17.0.1)" value="<?php echo $dockerip; ?>">
        <span class="glyphicon fa fa-laptop form-control-feedback"></span>
      </div>
	<div class="form-group has-feedback" style="width: 350px">
        <input type="text" name="dockermask" class="form-control" placeholder="Container Netwrok Mask (Default: 255.255.255.0)" value="<?php echo $dockermask; ?>">
        <span class="glyphicon fa fa-laptop form-control-feedback"></span>
      </div>
              <br>

              <button type="submit" name="updatedecoysubnet" value="Yes" class="btn btn-primary">Update & Reboot</button>
                </div>
              </form>
              </div>
	  ...
	  <div class="box box-primary">
		...
		exec("sudo /bin/cat /etc/resolv.conf|grep -i nameserver|awk -F \"nameserver \" '{print$2}'",$outputdns,$result);
