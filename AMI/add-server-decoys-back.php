...
$val = getopt(null, ["interface:", "decoyname:", "decoygroup:", "SMB:", "FTP:", "SSH:", "TELNET:", "RDP:", "VNC:", "TFTP:", "MSSQL:", "MYSQL:", "MODBUS:", "SNMP:", "S7COMM:", "WEB:", "webservertype:", "apachedecoyfile:", "smbdecoyfile:", "customssh:", "ttl:", "decoy_type:", "customrdp:", "deviceid:", "realRDPIP:"]);
...
include "db.php";
	...
	$ipcheckloop=0;
    	...
    	{
		global $ipa, $mask, $GW;
		exec("sudo /sbin/ifconfig $interface| grep -i inet| grep -i netmask|awk -F \" \" '{print$2}'| grep -v ^\"169.254.\" |grep [0-9]",$output1,$result);
                exec("sudo /sbin/ifconfig $interface| grep -i inet| grep -i netmask|awk -F \" \" '{print$4}'|grep [0-9]",$output4,$result);
                exec("sudo /sbin/route -n| grep -i $interface|awk -F \" \" '{print$2}'|grep -v \"0.0.0.0\"|grep [0-9]",$output5,$result);
	...
	while($networkaddcheck == 0)
		...
		exec("sudo /usr/bin/sipcalc $interface|grep -i \"Network mask \"|grep -i \"bits\" |head -1|awk -F \"- \" '{print$2}'|xargs",$outputcidrmask,$result);
                exec("sudo /usr/bin/sipcalc $interface|grep -i \"Network address\"| head -1|awk -F \"- \" '{print$2}'",$outputnetworkadd,$result);
...
// Modify TTL based on provided input
	...
	$mysqli = db_connect();
        ...
        if ($stmt = $mysqli->prepare("INSERT INTO decoys (decoyname,decoygroup,services,interface,ipaddress,ttl,decoy_type) VALUES (?, ?,'',?,?,?,?)"));{
        $stmt->bind_param("sssssi", $decoyname,$decoygroup,$interface,$ipa,$ttl,$decoy_type);
        $stmt->execute();
        $stmt->close();
                }
		...
		if ($smbdecoyfile == true)
    		...
    		exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"smbd\" \"$output3[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
    	...
    	if ($FTP == 'on')
    	{
            ...
            //
		exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"ftpd\" \"$output9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
        ...
        if ($SSH == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"$sshdecoyimage\" \"$outputssh9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
       ...
       if ($VNC == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"vnc\" \"$outputvnc9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
      	...
      	if ($TFTP == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"tftp\" \"$outputtftp9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
  	...
  	if ($TELNET == 'on')
            ...
            // 
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"telnet\" \"$outputtelnet9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
       ...
       if ($MSSQL == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"mssql\" \"$outputmssql9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
        ...
        if ($RDP == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"$rdpdecoyimage\" \"$outputrdp9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
    	...
    	if ($MYSQL == 'on')
            ...
            // 
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"mysql\" \"$outputmysql9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
    	...
    	if ($WEB == 'on')
            ...
            // 
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"tomcat\" \"$outputtomcat9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
    		}
		...
		if ($apachedecoyfile == true)
            ...
            // 
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"apache\" \"$outputapache9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
                    }
		...
		if ($webservertype == 'basicauth')
            ...
            //
                  ...
                  //
                  exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"basicauth\" \"$outputtomcat9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
                }
        ...
        if ($MODBUS == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"modbus\" \"$outputmodbus9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
        ...
        if ($SNMP == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"snmp\" \"$outputsnmp9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
      ...
      if ($S7COMM == 'on')
            ...
            //
                    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"s7comm\" \"$outputs7comm9[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" \"$deviceid\" > /dev/null 2>&1 &",$output,$result);
