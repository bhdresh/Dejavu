...
if($apiStatus == 1)
{
	...
	{	
		...
		$data = array('auth_key' => $apikey, 'deviceid' => $deviceid, 'Decoy_Name' => $decoyName, 'Decoy_Group' => $decoyGroup, 'Decoy_IP' => $decoyIP,'Attacker_IP' => $attackerIP, 'EventType' => $eventType, 'Raw_Logs' => $rawlog, 'Service_Name' => $serviceType, 'Timestamp' => $logtime, 'decoy_type' => $decoy_type, 'pcap_filename' => $pcap_filename, 'video_filename' => $video_filename, 'msg_filename' => $msg_filename);
