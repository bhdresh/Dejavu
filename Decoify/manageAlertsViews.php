...
var new_filter =`<div class="margin" id="advanceSearch">
    ...
    <input type="text" id="search_data" style="width:200px; height: 30px" name="search_data" placeholder="Enter Here">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        ...
        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#modal-default-alert">
        Default Alert
        </button>
      </h1>
    </section>
...
<section class="content">
      <div class="row">
        ...
        <div class="col-xs-12">
                  ...
                  <div class="modal-body">
                    ...
                    <form id="formSearch" name="formSearch" action="manageAlerts.php" method="POST">
                      ...
                      <div class="margin" id="advanceSearch">
                          ...
                          <input type="text"  style="width:200px; height: 30px" name="search_data" placeholder="Enter Here" id="search_data">
           ...
           <div class="modal fade in" id="modal-default-alert" style="display: none;">
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">×</span></button>
                                    <h4 class="modal-title">Modify Default Alert</h4>
                                  </div>
                                  <div class="modal-body">
                                    <?php foreach ($default_event as $key => $value): ?>
                                     <?php $alert_info = $default_event[$key]['Alert_Info'];
                                         $alert = json_decode($alert_info, true);
                                          //print_r(array_values($alert));
                                          //echo $alert[0]['filter'];
                                         $alert_id = $default_event[$key]['id'];
                                    ?>
                                    <form id="formSearch<?= $alert_id?>" name="formSearch" action="manageAlerts.php" method="POST">
                                      ...
                                      <div class="form-group">
                                        <label for="exampleInputEmail1">Email address</label>
                                        <input type="email" class="form-control" name="email_to" id="email_to" value="<?= dataFilter($default_event[$key]['Email_To'])?>">
                                      </div>

                                      <div class="form-group">
                                        <label for="exampleInputEmail1">Alert Name</label>
                                        <input type="email" class="form-control" name="alert_name" id="alert_name" value="<?= dataFilter($default_event[$key]['Alert_Name'])?>">
                                      </div>

                                      <div class="form-group">
                                        <label for="exampleInputEmail1">Description</label>
                                        <input type="email" class="form-control" name="alert_desc" id="alert_desc" value="<?= dataFilter($default_event[$key]['Alert_Desc'])?>">
                                      </div>
                                      <div class="margin">
                                        <p>Match 
                                          <select name="filter" id="filter<?= $alert_id?>">
                                          <option value="none" <?php if ($alert[0]['filter'] == 'none') { echo ' selected="selected"'; } ?> >none
                                          </option>
                                    ...
                                    <?php foreach ($alert[0]['alertCriteria'] as $alertCriteria): ?>
                                      <div class="margin" id="editAlert<?= $alert_id?>">
                                          ...
                                          <select name="condition" id="condition<?= $alert_id?>">
                                            ...
                                            <option value="not_eq" <?php if ($alertCriteria['condition'] == 'not_eq') { echo ' selected="selected"'; } ?> >Not equal to</option>
                                          </select>
                                          <input type="text"  style="width:200px; height: 30px" name="search_data" value="<?= dataFilter($alertCriteria['search_data'])?>" id="search_data<?= $alert_id?>">
                                          <i class="fa fa-fw fa-plus add-filter"></i>
                                      </div>
                                    <?php endforeach;?>                    
                                  </div>
                                  <div class="modal-footer">
                                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
                                    <button type="button" class="btn btn-primary" id="advSearchSubmit" onclick="modifyAlert(<?= $alert_id?>)">Modify Default Alert</button>
                                  </div>
                                <?php endforeach;?>  
                                </form>
                                </div>
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <?php foreach ($alert[0]['alertCriteria'] as $alertCriteria): ?>
                                      <div class="margin" id="editAlert<?= $alert_id?>">
                                          ...
                                          <input type="text"  style="width:200px; height: 30px" name="search_data" value="<?= $alertCriteria['search_data']?>" id="search_data<?= $alert_id?>">
                                          <i class="fa fa-fw fa-plus add-filter"></i>
                                      </div>
                                    <?php endforeach;?>                    
                                  </div>
